...
class ApiUndangan {
  final url = "http://127.0.0.2:8080/undangan/getListUndangan.php";
  ...
  Future<Undangan?> cekUndangan(String email) async {
    final response = await http.get(Uri.parse(
        "http://127.0.0.2:8080/undangan/cekUndangan.php?email=$email"));
    if (response.statusCode == 200) {
  ...
  Future<bool> updateKehadiran(Undangan undangan) async {
    final response = await http.get(Uri.parse(
        "http://127.0.0.2:8080/undangan/updateKehadiran.php?undangan_id=${undangan.undanganId}"));
    if (response.statusCode == 200)
      return true;
    else
      return false;
  ...
  Future<bool> resetKehadiran() async {
    final response = await http
        .get(Uri.parse("http://127.0.0.2:8080/undangan/resetKehadiran.php"));
    if (response.statusCode == 200)
      return true;
    else
      return false;
